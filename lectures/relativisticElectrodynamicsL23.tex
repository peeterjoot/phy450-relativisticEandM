...
\end{equation}
%
This is the Maxwell stress tensor, which has diagonal entries
\begin{dmath}\label{eqn:relativisticElectrodynamicsL23:251}
T^{\alpha\alpha}
=
\inv{4 \pi}
\begin{bmatrix}
\inv{2}(\BE^2 + \BB^2) \\
-\left(E_x^2 + B_x^2 - \inv{2}(\BE^2 + \BB^2)\right) \\
-\left(E_y^2 + B_y^2 - \inv{2}(\BE^2 + \BB^2)\right) \\
-\left(E_z^2 + B_z^2 - \inv{2}(\BE^2 + \BB^2)\right) \\
\end{bmatrix},
\end{dmath}
...
\begin{dmath}\label{eqn:relativisticElectrodynamicsL23:250}
\inv{4 \pi}
\begin{bmatrix}
. &  (\BE \cross \BB)^x &  (\BE \cross \BB)^y              &  (\BE \cross \BB)^z              \\
. & .                   & -\left(E_x E_y + B_x B_y \right) & -\left(E_x E_z + B_x B_z \right) \\
. & .                   & .                                & -\left(E_y E_z + B_y B_z \right) \\
. & .                   & .                                & .
\end{bmatrix}.
\end{dmath}
%  I had be curious what his arguments were and how he related this to the Lorentz force?
%
In words this matrix is
\begin{equation}\label{eqn:relativisticElectrodynamicsL23:270}
\begin{bmatrix}
\text{en. density} & \inv{c} (\text{en. flux in \(\xcap\)}) & \inv{c} (\text{en. flux in \(\ycap\)}) &
%\inv{c} (\text{en. flux in \(\zcap\)}) \\
\cdots \\
c \times (\text{mom. density})^x
& (\text{mom.})^x \text{flux in \(\xcap\)}
& (\text{mom.})^x \text{flux in \(\ycap\)}
&
%(\text{mom.})^x \text{flux in \(\zcap\)}
\cdots
\\
c \times (\text{mom. density})^y
& (\text{mom.})^y \text{flux in \(\xcap\)}
& (\text{mom.})^y \text{flux in \(\ycap\)}
&
%(\text{mom.})^y \text{flux in \(\zcap\)} \\
\cdots
\\
c \times (\text{mom. density})^z
& (\text{mom.})^z \text{flux in \(\xcap\)}
& (\text{mom.})^z \text{flux in \(\ycap\)}
&
%(\text{mom.})^z \text{flux in \(\zcap\)} \\
\cdots
\\
\end{bmatrix}.
\end{equation}
Maxwell apparently derived this without any use of four vectors or symmetry arguments.
%
