...
\end{equation}
%
provided these transform under coordinate transformations like the products of components of two four vectors.
They also provide raising and lowering rules that distinguish the quantities \({A^{\mu}}_\nu\), and \({A_{\mu}}^\nu\) by relating these to the raising and lowering operations so that, for example, \({A_0}^1 = A^{01}\), \({A^0}_1 = -A^{01}\).
This is consistent with the notation I have used in a blundering fashion.
This also highlights the difference between \({L_\mu}^\nu\), and \({L^\nu}_\mu\).
We can relate both of these back to the index upper tensor representation
%
...
da_1 da_2 \cdots da_k \\
&=
\inv{(k-1)!} \epsilon^{ r s \cdots t u } \\
&\quad\int da_u \PD{a_{u}}{} F_{\nu_1 \nu_2 \cdots \nu_{k-1} }
\Abs{\frac{\partial(
x^{\nu_{k-1}},x^{\nu_{k-2}},\cdots,x^{\nu_1}
)}{\partial(a_r, a_s, \cdots, a_t)}}
da_r da_s \cdots da_t.
