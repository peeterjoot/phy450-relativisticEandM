...
%\date{Feb 15, 2011}
%
\makeproblem{Fun with \( \epsilon_{\alpha\beta\gamma}, \epsilon^{ijkl}, F_{ij} \), and Maxwell duality.}{pr:relElectroDynProblemSet3:1}{
%
...
In class Simon showed us how the first identity can be arrived at using the triple product \(\Ba \cdot (\Bb \cross \Bc) = \Det(\Ba \Bb \Bc)\).  It occurred to me later that I had seen the identity to be proven in the context of Geometric Algebra, but had not recognized it in this tensor form.  Basically, a wedge product can be expanded in sums of determinants, and when the dimension of the space is the same as the vector, we have a pseudoscalar times the determinant of the components.
...
The \R{4} case follows exactly the same way, where we have
%
